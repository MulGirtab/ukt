...
local mincash = 10000 -- minimum amount of cash a pile holds
local maxcash = 750000 -- maximum amount of cash a pile can hold
...
QBCore.Functions.CreateCallback("utk_oh:GetData", function(source, cb)
    cb(info)
end)
QBCore.Functions.CreateCallback("utk_oh:GetDoors", function(source, cb)
    cb(PoliceDoors)
end)
QBCore.Functions.CreateCallback("utk_oh:startevent", function(source, cb, method)
    local xPlayers = QBCore.Functions.GetPlayers()
    local copcount = 0
    local yPlayer = QBCore.Functions.GetPlayer(source)
    ...
    if not info.locked then
        if (os.time() - cooldown) > lastrobbed then
            ...
            if copcount >= mincops then
                if method == 1 then
                    local item = yPlayer.Functions.GetItemByName("thermal_charge")["count"]

                    if item >= 1 then
                        yPlayer.Functions.RemoveItem("thermal_charge", 1)
                        cb(true)
                        info.stage = 1
                        info.style = 1
                        info.locked = true
                ...
                elseif method == 2 then
                    local item = yPlayer.Functions.GetItemByName("lockpick")["count"]

                    if item >= 1 then
                        yPlayer.Functions.RemoveItem("lockpick", 1)
                        info.stage = 1
                        info.style = 2
                        info.locked = true
                        cb(true)
...
QBCore.Functions.CreateCallback("utk_oh:checkItem", function(source, cb, itemname)
    local xPlayer = QBCore.Functions.GetPlayer(source)
    local item = xPlayer.Functions.GetItemByName(itemname)["count"]
...
QBCore.Functions.CreateCallback("utk_oh:gettotalcash", function(source, cb)
    cb(totalcash)
end)
