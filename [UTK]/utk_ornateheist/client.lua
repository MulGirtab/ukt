...
function UTK:GetInfo()
    QBCore.Functions.TriggerCallback("utk_oh:GetData", function(output)
        self.info = output
        self:HandleInfo()
    end)
end
						...
						local coords = GetEntityCoords(PlayerPedId())
							...
							if dst <= 6 then
								...
								if dst <= 1 and IsControlJustReleased(0,38) then
									QBCore.Functions.TriggerCallback("utk_oh:startevent", function(output)
										if output == true then
											UTK.initiator = true
											self.info.stage = 1
											self.info.style = 1
										...
										elseif output ~= true then
											exports["QBCore:Notify"]:SendAlert("error", output)
										end
									end, 1)
								end
							end
							...
							while true do
								...
								if not UTK.checks.hack1 then
									...
									if dst <= 4 then
										...
										if dst <= 1 and IsControlJustReleased(0, 38) then
											QBCore.Functions.TriggerCallback("utk_oh:checkItem", function(output)
												if output then
													UTK.checks.hack1 = true
													self.currenthack = 0
													self:Hack()
												elseif not output then
													exports['QBCore:Notify']:SendAlert("error", "You don't have a hacker laptop.")
												end
											end, "hackerDevice")
										end
									end
									...
									if dst <= 4 then
										...
										if dst <= 1 and IsControlJustReleased(0, 38) then
											...
											if output then
												...
												self:IdCard()
											elseif not output then
												exports["QBCore:Notify"]:SendAlert("error", "You don't have the ID Card.")
											end
											end, "id_card")
										end
									end
										...
										if dst <= 1 and IsControlJustReleased(0, 38) then
											QBCore.Functions.TriggerCallback("utk_oh:checkItem", function(output)
												if output then
													...
													self:Plant()
												elseif not output then
													exports['QBCore:Notify']:SendAlert("error", "You don't have any thermal charges.")
												end
											end, "thermal_charge")
										end
									end
											...
											if dst <= 1 and IsControlJustReleased(0, 38) then
												QBCore.Functions.TriggerCallback("utk_oh:checkItem", function(output)
													if output then
														...
														self:Lockpick()
													elseif not output then
														exports['QBCore:Notify']:SendAlert("error", "You don't have any lockpicks.")
													end
												end, "lockpick")
											end
										end
...
function UTK:Plant()
    ...
    exports['QBCore:Notify']:SendAlert("success", self.text.melted)
...
function UTK:Lockpick()
    ...
    Citizen.Wait(6500)
    exports['QBCore:Notify']:SendAlert("success", "Stage 1 "..self.text.stage)
    SetEntityCoords(ped, loc.x, loc.y, loc.z, 1, 0, 0, 1)
    SetEntityHeading(ped, loc.h)
    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 6000, 1, 1, 0, 0, 0)
    exports['progressBars']:startUI(6000, self.text.lockpick..": Stage 2")
    Citizen.Wait(6500)
    exports['QBCore:Notify']:SendAlert("success", "Stage 2 "..self.text.stage)
    exports['QBCore:Notify']:SendAlert("success", self.text.unlocked)
    UTK.disableinput = false
    if self.currentpick == 0 then
        UTK.stage0break = true
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    exports["QBCore:Notify"]:SendAlert("success", "Open My Computer and navigate to HackConnect.exe")
...
function UTK:IdCard()
    ...
    if self.currentid == 1 then
        UTK.disableinput = false
        exports['QBCore:Notify']:SendAlert("success", UTK.text.unlocked)
    ...
    elseif self.currentid == 2 then
        ...
        repeat
            ...
            count = count - 1
        until count == 0
        exports['QBCore:Notify']:SendAlert("success", UTK.text.used)
        SpawnObj()
...
function Search(location)
    ...
    if UTK.searchinfo.random ~= 1 then
        ...
        exports['QBCore:Notify']:SendAlert("error", UTK.text.nothing)
        UTK.searchinfo.random = math.random(1, UTK.cur - 1)
        UTK.cur = UTK.cur - 1
    else
        UTK.searchinfo.found = true
        exports['QBCore:Notify']:SendAlert("success", UTK.text.found)
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    exports["QBCore:Notify"]:SendAlert("success", "Find the IP adress...")
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    exports["QBCore:Notify"]:SendAlert("success", "Find the password...")
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    exports["QBCore:Notify"]:SendAlert("success", "Run BruteForce.exe")
                ...
                elseif Hacking and program == 86 then
                    ...
                    if UTK.hackmethod == 1 then
                        --TriggerServerEvent("utk_oh:toggleDoor", UTK.hack1.type, vector3(UTK.hack1.x, UTK.hack1.y, UTK.hack1.z), false, UTK.hack1.h)
                        TriggerServerEvent("utk_oh:policeDoor", 3, false)
                        exports['QBCore:Notify']:SendAlert("success", UTK.text.hacked)
                        UsingComputer = false
                        UTK.disableinput = false
                        UTK.hackfinish = true
                        Ipfinished = false
...
RegisterNetEvent('utk:setJob')
AddEventHandler('utk:setJob', function(job)
    PlayerData.job = job
    UTK.stage0break = true
    UTK.stage1break = true
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['QBCore:Notify']:SendAlert("error", "You have 90 seconds until toxic gas comes in.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
        ...
        QBCore.Functions.TriggerCallback("utk_oh:gettotalcash", function(result)
            text = "$"..ESX.Math.GroupDigits(result)
            exports["QBCore:Notify"]:SendAlert("success", "You stole "..text)
        end)
    else
        exports['QBCore:Notify']:SendAlert("error", "You have 90 seconds until toxic gas comes in.")
        UTK.starttimer = true
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports["QBCore:Notify"]:SendAlert("error", "Vault door will close automatically in 60 seconds!")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                exports["QBCore:Notify"]:SendAlert("error", "VAULT DOOR CLOSING!")
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
		local coords      = GetEntityCoords(PlayerPedId())
        if toggle == 1 then	
			exports["QBCore:Notify"]:SendAlert("infrom", "Pacific Standart Bank alarms are triggered!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
			exports['core_dispach']:addCall("20-12", "Bank Robbery", {{icon=fontawsomeicon, info="Dispatch: Pacific Standard Bank Alarm has been triggered"}}, {coords.x,coords.y,coords.z}, "police", 3000, 11, 5 )
			TriggerServerEvent('MF_Trackables:Notify','[NEW]Report: Pacific Standart Bank alarms are triggered.',GetEntityCoords(GetPlayerPed(-1)),'Police')
...
Citizen.CreateThread(function()
   --[[ while ESX == nil do
    ...
    QBCore.Functions.TriggerCallback("utk_oh:GetDoors", function(result)
        PoliceDoors = result
    end)
    ...
    while true do
        ...
        for k, v in ipairs(PoliceDoors) do
            ...
            if dst <= 50 then
                ...
                if PlayerData.job.name == "police" or PlayerData.job.name == "banker" then
                    if dst <= 1.5 then
